...
internal static partial class InitialData
{
    public static Recipe[] Recipes(Ingredient[] ingredients)
    {
        ...
        var addBacon = AddIngredientAction.Create(bacon, 1.0m, MeasureUnit.Package).Value;
        
        var onion = ingredients.Single(i => i.Name == "Cebula czerwona, średnia");
        var addOnion = AddIngredientAction.Create(onion, 1.0m, MeasureUnit.Piece).Value;
        
        var jalapeno = ingredients.Single(i => i.Name == "Jalapeno, świeże");
        var addJalapeno = AddIngredientAction.Create(jalapeno, 1.0m, MeasureUnit.Piece).Value;
        
        var garlic = ingredients.Single(i => i.Name == "Ząbki czosnku");
        var addGarlic = AddIngredientAction.Create(garlic, 1.0m, MeasureUnit.Piece).Value;
        
        var olives = ingredients.Single(i => i.Name == "Oliwki zielone");
        var addOlives = AddIngredientAction.Create(olives, 10.0m, MeasureUnit.Gram).Value;
        
        var chicken = ingredients.Single(i => i.Name == "Pierś z kurczaka");
        var addChicken = AddIngredientAction.Create(chicken, 250.0m, MeasureUnit.Gram).Value;
        
        var mushrooms = ingredients.Single(i => i.Name == "Pieczarki");
        var addMushrooms = AddIngredientAction.Create(mushrooms, 3.0m, MeasureUnit.Piece).Value;

        List<Result<RecipeStep>> pizza1Steps =
        [
