...
{
    public static AddIngredientAction AddIngredient(Ingredient ingredient, decimal quantity, MeasureUnit unit) => AddIngredientAction.Create(ingredient, quantity, unit).Value;
    ...
    public static AddMealAction AddMeal(Recipe recipe, int order, int servings) =>
        AddMealAction.Create(recipe, order, servings).Value;
}